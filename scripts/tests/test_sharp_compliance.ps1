...
try {
    ...
    if ($skillsCount -ge 5) {
        ...
        foreach ($skill in $card.skills) {
            [string]$keywords = ""
            if ($skill.tags) { $keywords = $skill.tags -join ", " }
            if ($keywords.Length -gt 50) {
                $keywords = $keywords.Substring(0, 50) + "..."
            }
            Write-Host "   - $($skill.name): $keywords"
        }
        $testResults += @{ Name = "Skills Array"; Passed = $true; Critical = $true }
    } else {
        Write-Host "❌ Insufficient skills" -ForegroundColor Red
...
try {
    ...
    if ($supportsFHIR -and $supportsSHARP) {
        ...
        $versionsText = $card.capabilities.supportedFHIRVersions -join ", "
        Write-Host "   FHIR Versions: $versionsText"
...
try {
    ...
    if ($scopesCount -ge 3) {
        Write-Host "✅ FHIR scopes declared" -ForegroundColor Green
        foreach ($scope in $card.capabilities.fhirScopes) {
            Write-Host "   - $scope"
        }
        $testResults += @{ Name = "FHIR Scopes"; Passed = $true; Critical = $true }
    } else {
        Write-Host "❌ Insufficient FHIR scopes" -ForegroundColor Red
...
try {
    ...
    Write-Host "⚠️  Experimental capabilities test failed" -ForegroundColor Yellow
    $testResults += @{ Name = "Experimental Capabilities"; Passed = $false; Critical = $false }
}
Write-Host ""
...
try {
    ...
    if ($interfacesCount -ge 3) {
        Write-Host "✅ Multiple interfaces supported" -ForegroundColor Green
        ...
        Write-Host "❌ Insufficient interfaces" -ForegroundColor Red
...
try {
    ...
    if ($cardAlt.name) {
        ...
        Write-Host "⚠️  /v1/card endpoint not working" -ForegroundColor Yellow
...
Write-Host "Total: $($criticalPassed + $optionalPassed)/$total passed"
Write-Host ""

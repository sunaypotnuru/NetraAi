...
def execute_with_retry(query_fn, retries=5, delay=2):
    for i in range(retries):
        try:
            ...
            if i == retries - 1:
                raise e
            print(f"  [WARN] Supabase execution failed (attempt {i+1}/{retries}): {err_str}. Retrying in {delay}s...")
            time.sleep(delay)
...
async def main():
    ...
    time.sleep(2)  # allow async write to settle
    logs = execute_with_retry(lambda: supabase.table("audit_logs") \
        .select("table_name, new_data, created_at") \
        .in_("table_name", ["get_patient_fhir", "query_patient_timeline"]) \
    ...
    if logs:
        ...
        for log in logs:
            od = log.get("new_data", {}).get("output_data", {})
            phi_leak = any(
                isinstance(od.get(k), str) and "[REDACTED]" not in od.get(k, "")
                for k in ("name", "email", "phone", "address")
                if k in od
            )
            check(f"No PHI leak in {log['table_name']} output", not phi_leak)

    # ── 5. DB record counts ───────────────────────────────────────
    print("\n[5] Database record counts")
    patients_count  = execute_with_retry(lambda: supabase.table("fhir_patients").select("id", count="exact").execute().count)
    obs_count       = execute_with_retry(lambda: supabase.table("fhir_observations").select("id", count="exact").execute().count)
    audit_count     = execute_with_retry(lambda: supabase.table("audit_logs").select("id", count="exact").execute().count)

    mock_mode = os.getenv("ALLOW_MOCK_RESPONSES", "false").lower() == "true"
    min_patients = 0 if mock_mode else 100
    min_obs = 0 if mock_mode else 500

    check("fhir_patients table accessible", (patients_count is not None) and (patients_count or 0) >= min_patients,
          f"{patients_count} rows (minimum expected: {min_patients})")
    check("fhir_observations table accessible", (obs_count is not None) and (obs_count or 0) >= min_obs,
          f"{obs_count} rows (minimum expected: {min_obs})")

...
router = APIRouter(
    prefix="/api/v1/patient/emergency-contacts", tags=["Emergency Contacts"]
)
...
async def get_emergency_contacts(
    ...
    try:
        ...
        response = (
            ...
            .select(
                "id, related_user_id, relation, is_emergency_contact, created_at, updated_at, profiles_patient!related_user_id(full_name, phone)"
            )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to fetch emergency contacts"
        )
...
async def add_emergency_contact(
    ...
    try:
        ...
        if contact.is_primary:
            ...
            ).eq("primary_user_id", current_user.sub).eq(
        ...
        if existing_user.data and len(existing_user.data) > 0:
            ...
            if not new_profile.data:
                raise HTTPException(
                    status_code=500, detail="Failed to create contact profile"
                )
        ...
        if not relationship_entry.data:
            raise HTTPException(
                status_code=500, detail="Failed to create emergency contact"
            )
        ...
        return {
            ...
            "created_at": relationship_entry.data[0].get(
                "created_at", datetime.now().isoformat()
            ),
            "updated_at": None,
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to add emergency contact: {str(e)}"
        )
...
async def update_emergency_contact(
    ...
    try:
        ...
        if profile_updates:
            supabase.table("profiles_patient").update(profile_updates).eq(
                "id", related_user_id
            ).execute()
        ...
        relationship_data = (
            updated_relationship.data[0] if updated_relationship.data else {}
        )
        ...
        return {
            ...
            "created_at": relationship_data.get(
                "created_at", datetime.now().isoformat()
            ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update emergency contact: {str(e)}"
        )


@router.delete("/{contact_id}")
async def delete_emergency_contact(
    contact_id: str, current_user: TokenPayload = Depends(get_current_patient)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to delete emergency contact: {str(e)}"
        )
...
async def set_primary_contact(
    ...
    try:
        ...
        supabase.table("family_relationships").update({"is_primary_contact": False}).eq(
            "primary_user_id", current_user.sub
        ).eq("is_emergency_contact", True).execute()

        # Set this contact as primary
        supabase.table("family_relationships").update({"is_primary_contact": True}).eq(
            "id", contact_id
        ).execute()

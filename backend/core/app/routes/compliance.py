...
async def get_complaints(
    ...
    """Fetch all complaints (Admin only). Includes reporter identity and category names."""
    try:
        # Retrieve complaints as a flat list first to avoid schema join relationship failures
...
async def create_complaint(data: dict, current_user=Depends(get_current_user)):
    ...
    try:
        ...
        new_complaint = {
            ...
            "submitter_email": data.get("submitter_email", "anonymous@netra-ai.com"),
            "submitter_type": data.get("submitter_type", "patient"),
        }

...
Convert the following rough consultation notes into a structured JSON format matching the SOAP structure.
...
- "objective": clinical observations, vitals, examination findings, test results
- "assessment": diagnosis or clinical impression, differential diagnoses
- "plan": treatment plan, prescriptions, referrals, follow-up instructions
- "follow_up": Recommended timeframe for next visit
- "action_items": Array of strings for immediate next steps

Keep it professional, concise, and clinically appropriate. Use medical terminology where appropriate. Respond with ONLY the JSON object."""
    try:
        response = await ai_service.chat(
            prompt=prompt,
            system_prompt="You are a clinical documentation assistant. Respond in JSON.",
            format="json",
        )
        ...
        try:
            soap_data = json.loads(response["content"])
        except Exception:

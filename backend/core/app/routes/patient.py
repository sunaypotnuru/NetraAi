...
def validate_phone(phone: str) -> bool:
    """Validate phone number using phonenumbers library with international support."""
    try:
        if not phone:
            return False
        # Try to parse without region first (for international format with +)
        try:
            parsed = phonenumbers.parse(phone, None)
        except NumberParseException:
            # Fallback to India region for numbers without country code
...
async def get_dashboard(current_user: TokenPayload = Depends(get_current_patient)):
    ...
    try:
        ...
        # ROOT CAUSE FIX: Database permission errors should not cause a 500 crash.
        # Fallback to a safe, empty dashboard so the UI doesn't break.
        logger.error(f"Dashboard error (Falling back to empty data): {str(e)}")
        return {
            "profile": {
                "id": current_user.sub,
                "full_name": "Patient",
    ...
    try:
        ...
        for scan in scan_data_list:
            ...
            if img_url and isinstance(img_url, str) and not img_url.startswith("http"):
                # Use centralized signed URL generation
...
async def upload_scan(
    ...
    try:
        ...
        # Default fallback result with clear warning
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if ai_response.status_code == 200:
                    ...
                    if not all(field in ai_result for field in required_fields):
                        ...
                        ai_result["fallback_reason"] = "Invalid AI response format"
                    else:
                        # Mark as real AI result
            ...
            ai_result["fallback_reason"] = "AI service connection failed"
        except Exception as e:
            logger.error(f"AI Service error: {str(e)}")
            ai_result["fallback_reason"] = f"AI service error: {str(e)}"
        ...
        # Validate hemoglobin estimate range (medical range: 7-20 g/dL)
...
async def get_patient_pro_questionnaires(
    ...
    try:
        ...
        logger.error(f"Error fetching PRO questionnaires (Fail-soft): {e}")
        return []
...
async def submit_pro_questionnaire(
    ...
    try:
        ...
        logger.error(f"Error submitting PRO data (Fail-soft): {e}")
        return {"status": "error", "message": "Database permission denied"}
    ...
    try:
        ...
        try:
            if (
                ...
                logger.warning(
                    "Twilio not fully configured. SOS SMS broadcast skipped. Running in simulated mode."
                )
                supabase.table(Tables.NOTIFICATIONS).insert(
                    {
                        Col.Notifications.USER_ID: current_user.sub,
                        Col.Notifications.TYPE: "emergency_sos",
                        Col.Notifications.TITLE: "SOS Triggered (Simulated)",
                        Col.Notifications.MESSAGE: f"SMS broadcast simulated (Twilio unconfigured). Last Location: {maps_link}",
                    }
                ).execute()
                return {
                    "success": True,
                    "simulated": True,
                    "message": "SOS Alert Dispatched to Emergency Contacts (Simulated Mode)",
                }
        except HTTPException:
            raise

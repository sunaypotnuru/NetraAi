...
class MLServiceConfig:
    ...
    # Health check timeout (60 seconds to allow HF Space wake-up)
...
class MLService:
        ...
        try:
            ...
            if use_cache:
                await self.cache.set(
                    cache_key, prediction_result.model_dump(), ttl=self.config.CACHE_TTL
                )
    ...
    def get_health_status(
        ...
        if model_type:
            ...
            return result.model_dump() if result else {"status": "unknown"}

        return {
            model_type.value: result.model_dump()

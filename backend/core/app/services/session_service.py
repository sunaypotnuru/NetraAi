...
class SessionService:
    ...
    async def check_session_validity(self, session_token: str) -> bool:
        ...
        try:
            ...
            # ROOT CAUSE FIX: Database permission errors should not block valid JWT sessions.
            # Since we already verified the JWT signature before calling this service,
            # we can safely fallback to 'True' to keep the user logged in if the DB lookup fails.
            logger.error(
                f"Error checking session validity (Falling back to valid JWT): {e}"
            )
            return True
    ...
    async def terminate_all_user_sessions(
        ...
        Args:
            ...
            reason: Reason for termination (e.g. user_logout_all, admin_forced)

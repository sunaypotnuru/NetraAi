...
class MedicationReminderService:
    ...
    def _map_medication(self, medication: Dict[str, Any]) -> Dict[str, Any]:
        """Maps medication fields to support both legacy and new schemas"""
        if not medication:
            return medication
    ...
    def _map_medication_input(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Maps inputs from either legacy or new schema to DB schema"""
        mapped = data.copy()
        if "medication_name" in mapped:
            mapped[Col.Medications.NAME] = mapped["medication_name"]
        elif "name" in mapped:
            mapped[Col.Medications.NAME] = mapped["name"]

        if "time_slots" in mapped:
            mapped[Col.Medications.REMINDER_TIMES] = mapped["time_slots"]
        elif "reminder_times" in mapped:
            mapped[Col.Medications.REMINDER_TIMES] = mapped["reminder_times"]
    ...
    async def create_medication(
        ...
        """
        mapped_data = self._map_medication_input(medication_data)
        medication = {
            Col.Medications.ID: str(uuid4()),
            Col.Medications.PATIENT_ID: patient_id,
            Col.Medications.NAME: mapped_data.get(Col.Medications.NAME),
            Col.Medications.DOSAGE: mapped_data.get("dosage"),
        ...
        if response.data:
            return self._map_medication(response.data[0])
    ...
    async def get_medication(
        ...
        return self._map_medication(response.data[0]) if response.data else None
    ...
    async def update_medication(
        ...
        """
        mapped_updates = self._map_medication_input(update_data)
        mapped_updates[Col.Medications.UPDATED_AT] = datetime.now().isoformat()
    ...
    # â”€â”€â”€ Medication Logs â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            for reminder_time in reminder_times:
                ...
                try:
                    ...
                    if reminder_datetime <= end_time:
                        reminders.append(
                            {
                                "medication": self._map_medication(medication),

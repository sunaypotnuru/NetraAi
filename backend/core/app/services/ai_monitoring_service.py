        ...
        try:
            ...
            metrics = {
                ...
                "avg_response_time_ms": (
                    int(
                        sum(int(float(str(r.response_time_ms or 0))) for r in requests)
                        / total
                    )
                    if total > 0
                    else 0
                ...
                "avg_confidence_score": (
                    round(
                        sum(float(str(r.confidence_score or 0.0)) for r in requests)
                        / total,
                        3,
                    )
                    if total > 0
                ...
                "total_cost_usd": round(
                    sum(float(str(r.cost_usd or 0.0)) for r in requests), 6
                ),
            ...
            return [
                {
                    ...
                    "success_rate": (
                        round(
                            float(getattr(m, "successful_requests"))
                            / float(getattr(m, "total_requests")),
                            3,
                        )
                    ...
                    "avg_confidence_score": (
                        float(getattr(m, "avg_confidence_score"))
                        if getattr(m, "avg_confidence_score") is not None
                        else 0.0
                    ),
                    "total_tokens": getattr(m, "total_tokens"),
                    "total_cost_usd": (
                        float(getattr(m, "total_cost_usd"))
                        if getattr(m, "total_cost_usd") is not None
                        else 0.0
                    ),
                }
                for m in metrics
    ...
    def _update_daily_metrics(self, model_name: str):
        ...
        try:
            ...
            avg_response_time = int(
                sum(
                    int(float(str(getattr(r, "response_time_ms") or 0)))
                    for r in requests
                )
                / total
            )
            avg_confidence = (
                sum(float(str(getattr(r, "confidence_score") or 0.0)) for r in requests)
                / total
            )
            total_tokens = sum(getattr(r, "total_tokens") or 0 for r in requests)
            total_cost = sum(
                float(str(getattr(r, "cost_usd") or 0.0)) for r in requests
            )
        ...
        try:
            ...
            for (model_name,) in models:
                ...
                total_successful = sum(
                    getattr(m, "successful_requests") for m in metrics
                )
                ...
                comparisons.append(
                    {
                        ...
                        "avg_response_time_ms": (
                            int(
                                sum(
                                    float(getattr(m, "avg_response_time_ms"))
                                    * float(getattr(m, "total_requests"))
                                    for m in metrics
                                )

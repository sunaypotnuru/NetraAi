    ...
    def check_rate_limit(
        ...
        try:
            ...
            if limits["cpd"] is not None:
                if (
                    float(getattr(rate_limit, "cost_today") or 0.0) + estimated_cost
                    > limits["cpd"]
                ):
                    remaining = float(limits["cpd"]) - float(
                        getattr(rate_limit, "cost_today") or 0.0
                    )
                    return (
                        False,
    ...
    def record_request(self, user_id: str, tokens_used: int, cost: float) -> bool:
        ...
        try:
            ...
            setattr(
                rate_limit,
                "requests_today",
                int(getattr(rate_limit, "requests_today") or 0) + 1,
            )
            setattr(
                rate_limit,
                "tokens_today",
                int(getattr(rate_limit, "tokens_today") or 0) + tokens_used,
            )
            setattr(
                rate_limit,
                "cost_today",
                float(getattr(rate_limit, "cost_today") or 0.0) + cost,
            )
        ...
        try:
            ...
            if limits["cpd"] is not None:
                remaining["cost_usd"] = max(
                    0.0,
                    limits["cpd"]
                    - float(str(getattr(rate_limit, "cost_today") or 0.0)),
                )
            else:

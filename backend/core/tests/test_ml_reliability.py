...
from app.utils.retry import retry_with_backoff, RetryConfig, sync_retry_with_backoff
...
from app.services.ml_service import MLService, MLModelType, MLModelStatus
...
class TestCircuitBreaker:
    ...
    async def test_circuit_breaker_fails_fast_when_open(self):
        ...
        config = CircuitBreakerConfig(
            failure_threshold=2, recovery_timeout=10.0  # Long timeout
        )
        breaker = CircuitBreaker("test-service", config)
...
class TestMLService:
    ...
    async def test_ml_service_fallback_result(self):
        ...
        result = service._create_fallback_result(
            MLModelType.ANEMIA, error="Service unavailable", response_time_ms=100.0
        )
...
class TestIntegration:
    ...
    async def test_circuit_breaker_with_retry(self):
        ...
        breaker = CircuitBreaker(
            "test-service", CircuitBreakerConfig(failure_threshold=3)
        )

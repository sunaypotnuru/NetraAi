...
class TestSessionTimeout:
    ...
    async def test_update_activity_extends_session(self, session_service):
        ...
        with patch("app.services.session_service.supabase") as mock_supabase:
            ...
            mock_supabase.table.return_value.update.return_value.eq.return_value.execute.return_value.data = [
                {"user_id": "user123", "session_token": "token123", "is_active": True}
            ]
    ...
    async def test_terminate_session(self, session_service):
        ...
        with patch("app.services.session_service.supabase") as mock_supabase:
            mock_supabase.table.return_value.update.return_value.eq.return_value.execute.return_value.data = [
                {"is_active": False, "termination_reason": "manual"}
            ]

            success = await session_service.terminate_session(
                "token123", reason="manual"
            )
...
class TestLoginHistory:
    ...
    async def test_record_successful_login(self, login_history_service):
        ...
        with patch("app.services.login_history_service.supabase") as mock_supabase:
            mock_supabase.table.return_value.insert.return_value.execute.return_value.data = [
                {"user_id": "user123", "email": "test@example.com", "success": True}
            ]
    ...
    async def test_get_user_login_history(self, login_history_service):
        ...
        with patch("app.services.login_history_service.supabase") as mock_supabase:
            ...
            history = await login_history_service.get_user_login_history(
                "user123", limit=50
            )
    ...
    async def test_get_failed_login_attempts(self, login_history_service):
        ...
        with patch("app.services.login_history_service.supabase") as mock_supabase:
            mock_supabase.table.return_value.select.return_value.eq.return_value.eq.return_value.gte.return_value.execute.return_value.count = (
                3
            )

            count = await login_history_service.get_failed_login_attempts(
                "user123", hours=24
            )
    ...
    @pytest.mark.asyncio
    async def test_detect_suspicious_activity_high_failure_rate(
        self, login_history_service
    ):
...
class TestUserRateLimiting:
    ...
    def test_rate_limit_check_at_limit(self):
        ...
        from app.middleware.user_rate_limit import (

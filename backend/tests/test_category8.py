...
try:
    ...
    # Test valid response
    response = (
        "The patient has fever and headache symptoms requiring medical attention."
    )
...
try:
    ...
    rendered = symptom_template.render(
        symptoms="fever, headache", age="35", gender="female", medical_history="none"
    )
    assert "fever" in rendered
    print("✅ Default template rendered successfully")
...
try:
    ...
    print(
        f"✅ Context stats: {stats['message_count']} messages, {stats['total_tokens']} tokens"
    )
    ...
    print("✅ Context export/import successful")
...
try:
    ...
    prompt = template.render(
        symptoms="headache, fever", age="35", gender="female", medical_history="none"
    )
    print("✅ Prompt rendered from template")
    ...
    print(
        f"✅ Confidence score: {confidence:.2f} ({validation_service.get_confidence_level(confidence)})"
    )

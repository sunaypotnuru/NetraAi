...
try:
    ...
    offer_result = service.create_offer(
        "test-consultation", "user1", "test-sdp", "offer"
    )
    assert offer_result["success"] is True
    print("✅ Offer creation: PASSED")

    answer_result = service.create_answer(
        "test-consultation", "user2", "test-sdp", "answer"
    )
    assert answer_result["success"] is True
    print("✅ Answer creation: PASSED")

    # Test ICE candidate
    ice_result = service.add_ice_candidate(
        "test-consultation", "user1", "test-candidate"
    )
...
try:
    ...
    ws_route_count = len(
        [r for r in ws_router.routes if hasattr(r, "path") and "/ws/" in r.path]
    )

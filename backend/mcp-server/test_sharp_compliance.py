    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if "name" in card and "version" in card:
                print_success("Agent card accessible at /.well-known/agent-card.json")
...
async def test_experimental_capabilities(card: Dict[str, Any]) -> bool:
    ...
    if sharp_capability.get("value"):
        ...
        print_warning(
            "SHARP capability not in experimental section (may need POFastMCP)"
        )
        return False
...
async def test_supported_interfaces(card: Dict[str, Any]) -> bool:
    ...
    if len(interfaces) >= 3:
        ...
        for interface in interfaces:
            print(f"   - {interface.get('type') or interface.get('protocolBinding')}: {interface['url']}")
        return True
    else:
    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            response = await client.post(
                ...
                json={"messages": [{"role": "user", "content": "Check for fatigue"}]},
            )
            response.raise_for_status()
    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if "result" in result and "message" in result["result"]:
                ...
                print(
                    f"   FHIR context used: {metadata.get('fhir_context_used', False)}"
                )
...
async def main():
    ...
    if result:
        ...
        results.append(
            (
                ...
                await test_experimental_capabilities(card),
                False,
            )
        )
        results.append(
            ("Supported Interfaces", await test_supported_interfaces(card), True)
        )

    # Test 7-10: Runtime tests
    results.append(("Health Check", await test_health_check(), True))
    results.append(
        ("SHARP Chat Completions", await test_sharp_chat_completions(), True)
    )
    results.append(("SHARP JSON-RPC", await test_sharp_json_rpc(), True))
    results.append(
        ("Alternative Card Endpoint", await test_alternative_card_endpoint(), False)
    )
    ...
    critical_passed = sum(
        1 for name, passed, critical in results if passed and critical
    )
    critical_total = sum(1 for name, passed, critical in results if critical)
    optional_passed = sum(
        1 for name, passed, critical in results if passed and not critical
    )

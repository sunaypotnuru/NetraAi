...
async def screen_parkinsons(
    ...
    **kwargs,
    ...
    if ctx and patient_id:
        ...
        patient_id = (
            await ctx.get_state("current_patient_id") or patient_id or "unknown"
        )
    elif not patient_id:
        patient_id = "unknown"
    ...
    # 💎 ROBUST EXTRACTION
    severity = (
        ml_result.get("severity") or 
        ml_result.get("prediction") or 
        ml_result.get("status") or 
        ml_result.get("grade_name") or
        "Normal"
    )
    score = ml_result.get("updrs_score") or ml_result.get("score") or 0.0

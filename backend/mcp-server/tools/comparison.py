...
async def compare_diagnostic_history(
    ...
    **kwargs,
    ...
    timeline = await query_patient_timeline(
        ctx, patient_id, "DiagnosticReport", limit=10, **kwargs
    )
...
async def _compare_anemia(
    ...
    if change > 0.5:
        ...
        so_what = "🚨 CRITICAL: Oral iron failure. High risk for heart failure. Escalate to IV iron."
    else:
...
async def _compare_cataract(
    ...
    if change > 0:
        trend, emoji = "Progression", "📉"
        so_what = (
            "Cataract worsening. Surgery indicated if visual acuity drops below 20/50."
        )
    else:
        trend, emoji = "Stable", "➡️"
...
async def _compare_mental_health(
    ...
    if change < -5:
        ...
        so_what = (
            "🚨 URGENT: Worsening depression. Assess for suicidal ideation. Call 988."
        )
    else:
        trend, emoji = "Stable", "➡️"

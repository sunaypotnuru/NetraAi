...
async def diagnose_anemia(
    ...
    **kwargs,
    ...
    anemia_service_url = os.getenv(
        "ANEMIA_SERVICE_URL", "https://sunay-potnuru-netra-anemia.hf.space"
    )
    ...
    # 💎 VERIFIED EXTRACTION (from Model app/main.py)
    hb = ml_result.get("hemoglobin_level") or ml_result.get("hemoglobin_estimate") or 0.0
    severity = ml_result.get("severity") or _classify_anemia_severity(hb, gender)
    
    # 💎 Clinical Logic (Gender-specific thresholds)
    # severity is already classified by your model! we just mirror it.
    recommendation = _get_anemia_recommendation(hb, severity)
...
def _get_anemia_recommendation(hb: float, severity: str) -> str:
    ...
    return recs.get(sev, "Monitor closely.")

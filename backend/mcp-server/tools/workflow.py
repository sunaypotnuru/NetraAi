...
async def orchestrate_screening_workflow(
    ...
    **kwargs,
    ...
    # 💎 Mock Input Injection for E2E testing
    import os
    if os.getenv("ALLOW_MOCK_RESPONSES", "false").lower() == "true":
        if not input_data.get("image_url"):
            input_data["image_url"] = "https://example.com/sample-clinical-image.jpg"
        if not input_data.get("audio_url"):
            input_data["audio_url"] = "https://example.com/sample-clinical-audio.wav"
        ...
        try:
            if tool_name == "diagnose_anemia":
                ...
                await _try_comparison(
                    ctx, "anemia", patient_id, results, fhir_server, fhir_token
                )
                ...
                results["diagnostic_results"].append({"tool": tool_name, "result": res})
                await _try_comparison(
                    ctx, "cataract", patient_id, results, fhir_server, fhir_token
                )
                ...
                results["diagnostic_results"].append({"tool": tool_name, "result": res})
                await _try_comparison(
                    ctx, "dr", patient_id, results, fhir_server, fhir_token
                )
...
async def _try_comparison(
    ctx, diag_type, patient_id, results, fhir_server=None, fhir_token=None
):
...
def _determine_workflow(complaint: str) -> Dict:
    workflows = {
        ...
        "screening": {
            ...
            "tools": ["detect_cataract", "screen_diabetic_retinopathy", "diagnose_anemia"],
        },
    }
    ...
    for diag in results["diagnostic_results"]:
        if "result" in diag:
            ...
            if severity not in ["Normal", "Mild"]:
                recs.append(
                    f"{diag['tool'].replace('_', ' ').title()}: {severity} detected."
                )
...
def _generate_clinical_summary(results: Dict) -> str:
        ...
        if "error" in diag:
            ...
            if isinstance(result_data, dict):
                ...
                summary_lines.append(
                    f"  ✅ {tool_name}: {severity} (Confidence: {confidence:.1%})"
                )
            else:
    ...
    if results.get("fhir_context_used"):
        summary_lines.append("")
        summary_lines.append(
            "✅ SHARP-on-MCP: External FHIR context successfully integrated"
        )

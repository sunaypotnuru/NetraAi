#!/usr/bin/env python3
"""
NetraAI A2A Agent - Prior Authorization Automation (v2.0.0)
============================================================

This A2A agent automates the prior authorization workflow by:
1. Extracting clinical evidence from diagnostic results (5 AI models)
2. Matching payer requirements and criteria (8 insurance providers)
3. Generating CMS-0057-F compliant prior auth packets with FHIR R4 Bundles
4. Coordinating multiple diagnostic tools via chief-complaint routing
5. Performing longitudinal trend analysis (Stanford HAI 2026 methodology)

11 MCP Tools | 6 Agent Skills | CMS-0057-F Compliant
...
# Add project root and local directory to path
...
class NetraAIPriorAuthAgent(A2AAgent):
    ...
    def __init__(self):
        ...
        if not self.mcp_api_key:
            raise RuntimeError("MCP_API_KEY must be configured for the A2A agent")
    ...
    def _create_agent_card(self, card_data: Dict) -> AgentCard:
        ...
        # Create capabilities (v2.0.0: added push_notifications support)
        cap_data = card_data["capabilities"]
        capabilities = AgentCapabilities(
            streaming=cap_data.get("streaming", True),
            extended_agent_card=cap_data.get("extended_agent_card", True),
            multi_turn=cap_data.get("multi_turn", True),
            push_notifications=cap_data.get("push_notifications", False)
        )
    ...
    async def handle_task(self, task: Task) -> Message:
        ...
        try:
            ...
            # Route to appropriate handler (v2.0.0: added longitudinal_trend_analysis)
            ...
            elif task.skill_id == "longitudinal_trend_analysis":
                result = await self._handle_longitudinal_trend_analysis(task_data)
            else:
    ...
    async def _call_mcp_tool(self, tool_name: str, **kwargs) -> Dict:
        """Call MCP tool via HTTP API with exponential backoff retries for free-tier cold starts"""
        ...
        for attempt in range(1, max_retries + 1):
            try:
                logger.info(f"Calling MCP tool: {tool_name} (Attempt {attempt}/{max_retries}) with args: {kwargs}")
                response = await self.http_client.post(url, json=payload)
                response.raise_for_status()

                result = response.json()
                logger.info(f"MCP tool {tool_name} completed successfully on attempt {attempt}")
                return result
                ...
                if isinstance(e, httpx.HTTPStatusError):
                    ...
                    is_server_error = status_code in [502, 503, 504, 408, 429]
                ...
                if is_server_error and attempt < max_retries:
                    ...
                    logger.warning(
                        f"MCP tool call failed ({error_msg}): {tool_name}. "
                        f"Retrying in {delay:.1f}s (server may be spinning up on free-tier)..."
                    )
                    await asyncio.sleep(delay)
                else:
                    logger.error(
                        f"MCP tool call failed permanently on attempt {attempt}: {tool_name}. "
                        f"Error: {error_msg}"
                    )
                    if isinstance(e, httpx.HTTPStatusError):
    ...
    async def _handle_clinical_evidence_extraction(self, task_data: Dict) -> Dict:
        ...
        try:
            ...
            if diagnostic_type == "anemia":
                ...
                audio_url = self._extract_parameter(task_data, "audio_url")
                result = await self._call_mcp_tool(
    ...
    async def _handle_multi_diagnostic_coordination(self, task_data: Dict) -> Dict:
        ...
        try:
            ...
            # Map workflow to workflow_type for test client compatibility
            if result and isinstance(result, dict) and "workflow" in result and "workflow_type" not in result:
                result["workflow_type"] = result["workflow"]
    ...
    async def _handle_longitudinal_trend_analysis(self, task_data: Dict) -> Dict:
        """Handle longitudinal trend analysis requests (v2.0.0 skill)"""
        try:
            patient_id = self._extract_parameter(task_data, "patient_id")
            diagnostic_type = self._extract_parameter(task_data, "diagnostic_type", "anemia")

            # Call the comparison tool for longitudinal trend analysis
            result = await self._call_mcp_tool(
            ...
            return {
                ...
                "diagnostic_type": diagnostic_type,
                "patient_id": patient_id,
                "research_backing": "Stanford HAI: Longitudinal EHR Datasets (2026)",
                "timestamp": datetime.now(timezone.utc).isoformat()
            }
            ...
            return {
                "status": "error",
                "message": f"Longitudinal trend analysis failed: {str(e)}",
                "timestamp": datetime.now(timezone.utc).isoformat()
            }

    async def _handle_general_query(self, task_data: Dict) -> Dict:
        """Handle general queries about the agent (v2.0.0)"""
        return {
            "status": "success",
            "agent_version": "2.0.0",
            "message": "NetraAI Prior Authorization Agent v2.0.0 — 11 MCP tools, 6 skills, CMS-0057-F compliant",
            "skills": [
                "prior_authorization_automation — CMS-0057-F compliant prior auth generation",
                "clinical_evidence_extraction — LOINC-coded FHIR evidence from 5 AI diagnostics",
                "payer_requirements_matching — 8 major insurance providers supported",
                "multi_diagnostic_coordination — Chief-complaint-driven workflow orchestration",
                "fhir_integration — R4 Patient, Observation, Bundle operations",
                "longitudinal_trend_analysis — Stanford HAI (2026) methodology, treatment failure detection"
            ],
            "mcp_tools": {
                "total": 11,
                "diagnostic_ml": ["diagnose_anemia", "detect_cataract", "screen_diabetic_retinopathy", "analyze_mental_health", "screen_parkinsons"],
                "fhir_operations": ["get_patient_fhir", "create_fhir_observation", "query_patient_timeline"],
                "advanced_analytics": ["compare_diagnostic_history", "orchestrate_screening_workflow", "generate_prior_auth"]
            },
            "supported_diagnostics": [
                "Anemia detection via conjunctiva analysis (WHO 2023)",
                "Cataract detection with XAI attention heatmaps (AAO 2024)",
                "Diabetic retinopathy screening with auto-urgency escalation (AAO 2024)",
                "Mental health assessment with crisis detection + 988 hotline (PHQ-9/DSM-5)",
                "Parkinson's screening via spiral drawing analysis (UPDRS/MDS)"
            ],
            "compliance": ["HIPAA", "FHIR R4", "CMS-0057-F (2026)", "LOINC coded"],
            "market_impact": {
    ...
    async def _run_diagnostic_screening(self, patient_id: str, service_requested: str) -> List[Dict]:
        ...
        elif "mental" in service_requested.lower():
            sample_audio = "https://example.com/sample-voice.wav"
            result = await self._call_mcp_tool(
                "analyze_mental_health_tool",
        ...
        elif "parkinson" in service_requested.lower():
            sample_image = "https://example.com/sample-spiral.jpg"
            result = await self._call_mcp_tool(
                "screen_parkinsons_tool",
                image_url=sample_image,
                patient_id=patient_id
    ...
    async def _extract_clinical_evidence(self, diagnostic_results: List[Dict], service_requested: str) -> Dict:
        ...
        for result in diagnostic_results:
            findings_dict = result.get("result", {}) if isinstance(result.get("result"), dict) else {}

            finding = result.get("conclusion")
            if not finding:
                if "hemoglobin_estimate" in findings_dict:
                    hb = findings_dict.get("hemoglobin_estimate")
                    unit = findings_dict.get("unit", "g/dL")
                    severity = findings_dict.get("severity", "Unknown")
                    finding = f"Hemoglobin estimate of {hb} {unit} indicating {severity} Anemia."
                elif "severity" in findings_dict:
                    finding = f"Diagnostic screening completed showing {findings_dict.get('severity')}."

            if finding:
                severity = result.get("clinical_decision_support", {}).get("severity")
                if not severity:
                    severity = findings_dict.get("severity") or "Unknown"

                recommendation = result.get("clinical_decision_support", {}).get("recommendation")
                if not recommendation:
                    recommendation = findings_dict.get("recommendation") or ""

                evidence_source = result.get("clinical_decision_support", {}).get("evidence_source")
                if not evidence_source:
                    evidence_source = findings_dict.get("guideline") or ""
    ...
    def _extract_clinical_evidence_from_result(self, result: Dict, diagnostic_type: str) -> Dict:
        ...
        if not finding:
            if "hemoglobin_estimate" in findings_dict:
                ...
                finding = "No conclusion available"
        ...
        if confidence is None:
            confidence = findings_dict.get("confidence", 0.95)
